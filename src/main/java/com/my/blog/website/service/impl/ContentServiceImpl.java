...
public class ContentServiceImpl implements IContentService {
    ...
    public PageInfo<ContentVo> getContents(Integer p, Integer limit) {
        ...
        example.setOrderByClause("top desc,created desc");
    ...
    @Override
    public Boolean importArticles(HttpServletRequest request, MultipartFile file) {
        Boolean flag = true;
        UserVo userVo = TaleUtils.getLoginUser(request);
        try {
            //第一个1代表sheet1, 第二个1代表从第几行开始读取数据，行号最小值为0
            List<Object> objectList = EasyExcelFactory.read(file.getInputStream(), new Sheet(1,
                    0, ContentVo.class));
            objectList.forEach(entity -> {
                if (entity instanceof ContentVo) {
                    ContentVo contentVo = (ContentVo) entity;
                    if (StringUtils.isNotBlank(contentVo.getContent()) &&
                            StringUtils.isNotBlank(contentVo.getTitle())) {
                        contentVo.setAuthorId(userVo.getUid());
                        contentVo.setType("post");
                        contentVo.setStatus("publish");
                        contentVo.setTags("Java");
                        contentVo.setAllowComment(true);
                        contentVo.setAllowFeed(true);
                        contentVo.setAllowPing(true);
                        this.publish(contentVo);
                    }
                }
            });
